...
layui.define(['code', 'element', 'table', 'util', 'carousel', 'laytpl'], function(exports){
  ...
  ;!function(){
    ...
    form.on('select(component)', function(data){
      ...
      if(/^(\/|http)/.test(value)){
        ...
        location.href = (url.pathname[1] === 'v1' ? '/docs/v1/doc/' : '/docs/v2/docs/') + value;
      }
  ...
  (function(){
    ...
    $('.layui-header .layui-nav-item').each(function(){
      ...
      if(pathname[1] === 'v2') pathname.splice(0, 1);
      if(pathname[1] && pathname[1] === othis.data('dir')){
  ...
  $(function(){
    ...
    demoCode.val([
      ...
      ,'<head>'
        ...
        ,'  <!-- 注意：项目正式环境请勿引用该地址 -->'
        ,'  <link href="//www.layuicdn.com/layui-v'+ layui.v +'/css/layui.css" rel="stylesheet">'
      ,'</head>'
      ,'<body>'
      ,global.preview
      ,'<!-- 注意：项目正式环境请勿引用该地址 -->'
      ,'<script src="//www.layuicdn.com/layui-v'+ layui.v +'/layui.js"></script>'
      ,$('#LAY_democodejs').html()
      ,'</body>\n</html>'
    ].join('\n'));
